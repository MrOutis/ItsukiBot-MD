...
if (!effect) throw 'Uso correcto del comamdo\n\n*Ejemplo:*\n*/logo 3d-deep-sea-metal* Itsuki\n\n*Ejemplo 2:*\n*/logo Wolf-Logo-Galaxy* Itsuki|Bot\n\n\t\t\t📝 *<Lista De Efectos>* 📝\n\n° ඬ⃟📝 /logo ' + effects.map(v => v.title).join('\n° ඬ⃟📝 /logo ')
effect = effect.toLowerCase()
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `Lo siento el efecto *${effect}* no se encuentra en la lista`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `Lo siento el efecto *${effect}* no se encuentra en la lista` : `Uso correcto del comamdo es\n*${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
let result = await axios.get(res, {
responseType: 'arraybuffer'
})
await conn.sendFile(m.chat, result.data, 'error.jpg', `*Toma tu logo!!*`, m)
}
handler.help = ['logos']
handler.tags = ['marker']
