        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                     this.reply(m.chat, `*Se agotaron tus diamantes 💎*`, m)
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`*⚠️ Reporte de comandos con fallas*\n\n*Plugin:* ${m.plugin}\n*Usuario:* ${m.sender}\n*Comando:* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\`\n\n`.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
...
global.dfail = (type, m, conn) => {
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: packname, body: 'Edited By おDaniel', thumbnail: miniurl, sourceUrl: script }}}}, { quoted: m })
